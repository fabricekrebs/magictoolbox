...
class ToolExecutionAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'id', 'tool_name', 'user', 'status', 'input_filename',
        'output_filename', 'duration_seconds', 'created_at'
    ]
    ...
    readonly_fields = [
        ...
        'duration_seconds', 'input_size', 'output_size'
    ]
    ordering = ['-created_at']
    
    fieldsets = (
        ('Basic Info', {
            'fields': ('id', 'user', 'tool_name', 'status')
        }),
        ('Files', {
            'fields': (
                'input_file', 'input_filename', 'input_size',
                'output_file', 'output_filename', 'output_size'
            )
        }),
        ('Parameters', {
            'fields': ('parameters',)
