...
class GPXKMLConverter(BaseTool):
    ...
    def validate(
        ...
        if file_ext != expected_format:
            return False, f"File extension .{file_ext} doesn't match conversion type (expected .{expected_format})"
    ...
    def process(
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=output_ext, mode='w', encoding='utf-8') as tmp_out:
            ...
            self.logger.info(f"Successfully converted {input_file.name} ({conversion_type}): {output_size / 1024:.1f} KB")
    ...
    def _convert_placemark_to_waypoint(self, placemark: ET.Element, gpx: ET.Element, ns: dict) -> None:
