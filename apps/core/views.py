...
def readiness_check(request):
    ...
    # Check Azure Function health and blob storage access
    azure_function_url = getattr(settings, "AZURE_FUNCTION_BASE_URL", None)
    if azure_function_url:
        try:
            import requests

            # Test function execution with health check (always detailed now)
            ...
            if response.status_code == 200:
                ...
                blob_status = health_data.get("blob_storage", {}).get("status")

                # Function is healthy if it responds and blob storage is connected
                checks["azure_function"] = blob_status == "connected"

                if blob_status != "connected":
                    logger.warning(f"Azure Function blob storage not accessible: {blob_status}")
            else:
                checks["azure_function"] = False
            ...
            try:
                ...
                for blob in blobs:
                    try:
                        ...
                        error_msg = (
                            f"Failed to delete blob {container_name}/{blob.name}: {str(blob_error)}"
                        )
