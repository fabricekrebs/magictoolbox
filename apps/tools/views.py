...
def delete_conversion(request, execution_id):
    ...
    try:
        ...
        if execution.output_file:
            try:
                ...
                if blob_service:
                    ...
                    upload_blob_client = blob_service.get_blob_client(container="uploads", blob=upload_blob_name)
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return JsonResponse({
                "success": True,
                "message": f"Deleted conversion for {input_filename}"
            })
        ...
        logger.warning(f"Conversion {execution_id} not found or user {request.user.email} not authorized")
    ...
    try:
        ...
        if total_count == 0:
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return JsonResponse({
                    "success": True,
                    "message": "No conversions to delete"
                })
    ...
    def download_result(self, request, pk=None):
        ...
        try:
            ...
            if not blob_client.exists():
                ...
                blob_client = blob_service.get_blob_client(container="processed", blob=alt_blob_name)
                if not blob_client.exists():
                    return Response(
