...
class GPXSpeedModifier(BaseTool):
    ...
    def process(
        ...
        try:
            ...
            if mode == 'analyze':
                ...
                with tempfile.NamedTemporaryFile(delete=False, suffix='.json', mode='w', encoding='utf-8') as tmp_out:
                ...
                with tempfile.NamedTemporaryFile(delete=False, suffix='.gpx', mode='w', encoding='utf-8') as tmp_out:
                    temp_output = tmp_out.name
                    tmp_out.write(modified_content)
            
            output_size = os.path.getsize(temp_output)
            self.logger.info(f"Successfully processed {input_file.name} ({mode} mode): {output_size / 1024:.1f} KB")
    ...
    def _analyze_gpx(self, root: ET.Element) -> Dict[str, Any]:
        ...
        if times:
            ...
            # Average speed
            if duration > 0:
                stats['average_speed_kmh'] = round((stats['total_distance_km'] / duration) * 3600, 2)
