...
def get_db_connection():
    ...
    logger.info(
        f"🔌 Connecting to database: {db_config['host']}:{db_config['port']}/{db_config['database']} as {db_config['user']}"
    )
    try:
...
def health_check(req: func.HttpRequest) -> func.HttpResponse:
    ...
    Always includes detailed connectivity test results.
    """
    ...
    # Always perform detailed checks:
    # Test blob storage access for all tool containers
    try:
        blob_service = get_blob_service_client()
        ...
        for container_name in containers_to_check:
            try:
                ...
                container_status[container_name] = "accessible" if exists else "not_found"
                if exists:
                    accessible_count += 1
            except Exception as e:
                container_status[container_name] = f"error: {str(e)[:50]}"

        health_status["blob_storage"] = {
            "status": "connected" if accessible_count > 0 else "error",
            "account": os.environ.get("AZURE_STORAGE_ACCOUNT_NAME", "unknown"),
            "containers_accessible": f"{accessible_count}/{len(containers_to_check)}",
            "containers": container_status,
        }
        logger.info(
            f"✅ Blob storage: {accessible_count}/{len(containers_to_check)} containers accessible"
        )
    except Exception as e:
        health_status["blob_storage"] = {"status": "error", "error": str(e)}
...
def rotate_video(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        output_blob_client = blob_service_client.get_blob_client(
            container="video-processed", blob=f"{execution_id}.mp4"
        )
...
def convert_pdf_to_docx(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        output_blob_client = blob_service.get_blob_client(
            container="pdf-processed", blob=f"{execution_id}.docx"
        )
        ...
        return func.HttpResponse(
            body=json.dumps(
                {"status": "success", "execution_id": execution_id, "output_blob": output_blob_name}
            ),
            mimetype="application/json",
            status_code=200,
        )
...
def _kml_to_gpx(kml_content: str) -> str:
    ...
    gpx = ET.Element(
        "gpx",
        {"version": "1.1", "creator": "MagicToolbox", "xmlns": "http://www.topografix.com/GPX/1/1"},
    )
...
def convert_gpx_kml(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        output_blob_client = blob_service.get_blob_client(
            container="gpx-processed", blob=f"{execution_id}{output_ext}"
        )
...
def convert_image(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        try:
            ...
            """,
                (
                    f"{execution_id}{output_ext}",
                    output_blob_full_path,
                    output_filename,
                    execution_id,
                ),
            )
            ...
            conn.close()
            logger.info(
                f"✅ Updated database with completed status, output_filename={output_filename}"
            )
        except Exception as db_error:
            logger.warning(f"⚠️  Database update failed: {db_error}")
...
def modify_gpx_speed(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        logger.info(
            f"⚡ GPX speed modification - ID: {execution_id}, multiplier: {speed_multiplier}x"
        )
        ...
        if speed_multiplier <= 0 or speed_multiplier > 100:
            return func.HttpResponse(
                body=json.dumps(
                    {"status": "error", "error": "Speed multiplier must be between 0 and 100"}
                ),
                mimetype="application/json",
                status_code=400,
            )
    ...
    for ts in timestamps:
        try:
            ...
            modified_content = modified_content.replace(
                f"<time>{ts}</time>", f"<time>{new_ts}</time>", 1
            )
        except Exception:
...
def list_blobs(req: func.HttpRequest) -> func.HttpResponse:
    """List all blobs in all tool containers."""
    try:
        ...
        return func.HttpResponse(
            body=json.dumps(result, indent=2), mimetype="application/json", status_code=200
        )

    except Exception as e:
        logger.error(f"❌ Error listing blobs: {e}")
        return func.HttpResponse(
            body=json.dumps({"error": str(e)}), mimetype="application/json", status_code=500
        )
...
def merge_gpx_files(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if not execution_id or file_count < 2:
            return func.HttpResponse(
                body=json.dumps(
                    {"status": "error", "error": "Missing execution_id or file_count < 2"}
                ),
                mimetype="application/json",
                status_code=400,
            )
